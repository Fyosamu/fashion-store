# Wander Lens - Cameras & travel store template

Camera shop built on repairs, trade-ins and loaner gear.

The complete Wander Lens store: one HTML page, one stylesheet, one script and 6 photographs.
...
- **Bag storage** - the cart key is `wander-lens\_bag` in localStorage, so two stores on the same domain
