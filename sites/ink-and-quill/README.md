# Ink & Quill - Stationery store template

Stationery shop with paper testing and free engraving.

The complete Ink & Quill store: one HTML page, one stylesheet, one script and 6 photographs.
...
| `img/` | 6 photographs (0.58 MB) - CC0, already compressed for the web |
...
- **Bag storage** - the cart key is `ink-and-quill\_bag` in localStorage, so two stores on the same domain
