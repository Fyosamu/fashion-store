# Bloom & Stem - Plants & flowers store template

Plant shop: 30-day guarantee, pet-safe range, Wednesday delivery.

The complete Bloom & Stem store: one HTML page, one stylesheet, one script and 6 photographs.
...
| `img/` | 6 photographs (0.8 MB) - CC0, already compressed for the web |
...
- **Bag storage** - the cart key is `bloom-and-stem\_bag` in localStorage, so two stores on the same domain
