# Maison Noir - Home & furniture store template

Furniture showroom: split editorial hero, workshop story, white-glove delivery copy.

The complete Maison Noir store: one HTML page, one stylesheet, one script and 6 photographs.
...
| `img/` | 6 photographs (0.5 MB) - CC0, already compressed for the web |
...
- **Bag storage** - the cart key is `maison-noir\_bag` in localStorage, so two stores on the same domain
