# Roastery No.5 - Specialty coffee store template

Coffee roastery: origins, subscriptions, gear and a roast-date promise.

The complete Roastery No.5 store: one HTML page, one stylesheet, one script and 6 photographs.
...
| `img/` | 6 photographs (0.56 MB) - CC0, already compressed for the web |
...
- **Bag storage** - the cart key is `roastery-no5\_bag` in localStorage, so two stores on the same domain
