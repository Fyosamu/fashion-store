# Glow Ritual - Clean skincare store template

Six-product skincare shop with clinical, fragrance-free messaging.

The complete Glow Ritual store: one HTML page, one stylesheet, one script and 6 photographs.
...
| `img/` | 6 photographs (0.36 MB) - CC0, already compressed for the web |
...
- **Bag storage** - the cart key is `glow-ritual\_bag` in localStorage, so two stores on the same domain
